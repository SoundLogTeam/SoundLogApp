...
function getPlaylistDescription(playlist?: LibraryPlaylistSummary) {
  return (
    ...
    [
      ...
      playlist?.trackCount ? `${playlist.trackCount}곡` : undefined,
    ]
      .filter(Boolean)
      .join(" · ") ||
...
export function LibraryScreen() {
  ...
  const { likedTracks, savedTracks, setLikeState, setSaveState } =
    useLibraryStore();
  ...
  const {
    data: remoteLibraryPages,
    fetchNextPage: fetchMoreRemoteLibrary,
    hasNextPage: hasMoreRemoteLibrary,
    isError: isRemoteLibraryError,
    isFetchingNextPage: isLoadingMoreRemoteLibrary,
    isLoading: isRemoteLibraryLoading,
    refetch: refetchRemoteLibrary,
    // `queryFn` must come first so TS can infer TQueryFnData from it before
    // typing `lastPage` in getNextPageParam below (object-literal generic
    // inference is order-sensitive here).
  } = useInfiniteQuery({
    queryFn: ({ pageParam }) => libraryApi.getTracks("all", pageParam),
    queryKey: ["library", "tracks", "all"],
    getNextPageParam: (lastPage: LibraryTracksPage) =>
      lastPage.page?.nextCursor ?? undefined,
    initialPageParam: undefined as string | undefined,
    staleTime: 5 * 60 * 1000,
  });
  // Pages come back in server (cursor) order; a track can only ever appear
  // in one page, but dedupe by id defensively so a refetch/retry overlap
  // never double-counts a row. Map preserves first-seen (i.e. server) order.
  const remoteLibraryRecords = useMemo(() => {
    const byId = new Map<string, RemoteLibraryTrackRecord>();

    remoteLibraryPages?.pages.forEach((page) => {
  ...
  const selectTab = (tab: LibraryTab) => {
    setSelectedTab(tab);
    setOpenTrackId(undefined);
    clearActionMessage();
  };
  const openPlaylist = (playlist: LibraryPlaylistRecord) => {
    const representativeRecord = playlist.records[0];

    if (representativeRecord) {
      setTrack(
        representativeRecord.track,
        playlist.playlistId,
        playlist.records.map((record) => record.track),
        playlist.playlist,
      );
    }

    router.push(`/playlist/${playlist.playlistId}` as never);
  };
  ...
  return (
    <Screen>
      ...
      >
        ...
        {isRemoteLibraryLoading && !hasLocalRecords ? null : selectedTab ===
          "playlists" ? (
                ...
                return (
                  ...
                  >
                    <View
                      ...
                      style={{
                        backgroundColor:
                          representativeRecord?.track.fallbackColor ??
                          "#2B176C",
                      }}
                    >
                      {imageUrl ? (
                    ...
                    <View className="ml-3 min-w-0 flex-1">
                      ...
                      <AppText
                        className="mt-1.5 text-[11px] text-white/34"
                        numberOfLines={1}
                      >
          ...
          <View className="mt-2">
            {records.map((record) => (
              <LibraryTrackRow
                key={record.track.id}
                isDeleteOpen={openTrackId === record.track.id}
                onRemove={() => {
                  setOpenTrackId(undefined);
                  removeRecord(record);
                }}
                onToggleDelete={() =>
                  setOpenTrackId((currentTrackId) =>
                    currentTrackId === record.track.id
                      ? undefined
                      : record.track.id,
                  )
                }
        ...
        {hasMoreRemoteLibrary ? (
          <Pressable
            accessibilityLabel="보관함 더 불러오기"
            accessibilityRole="button"
            accessibilityState={{
              busy: isLoadingMoreRemoteLibrary,
              disabled: isLoadingMoreRemoteLibrary,
            }}
            className="mt-4 h-12 items-center justify-center rounded-xl border border-white/10 bg-white/[0.06]"
            disabled={isLoadingMoreRemoteLibrary}
            onPress={() => void fetchMoreRemoteLibrary()}
          >
            {isLoadingMoreRemoteLibrary ? (
              <ActivityIndicator color="#B7E628" />
            ) : (
