...
function getMemberLabel(member: TravelRoomMember, index: number, currentUserId?: string) {
...
function getMemberCaption(member: TravelRoomMember, currentUserId?: string) {
  const name = member.userId === currentUserId ? '나' : member.displayName ?? '동행자';
...
function CandidateMomentRow({
  ...
  const visibleComments = commentsExpanded ? moment.comments ?? [] : latestComment ? [latestComment] : [];
      ...
      {canComment ? (
        <View className="mt-3 gap-2">
          ...
          <Pressable
            ...
            style={{ opacity: pendingComment || !commentDraft.trim() ? 0.55 : 1 }}
          >
...
export function CommunityRecapCard({
  ...
  const hiddenMomentCount = Math.max(allCandidateMoments.length - candidateMoments.length, 0);
  ...
  const memberOverflowCount = Math.max((room?.memberCount ?? 0) - (visibleMembers?.length ?? 0), 0);
  ...
  useEffect(() => {
    ...
    communityApi
      ...
      .catch(() => {
        if (!ignore) {
          setMessage('기존 공동 여행방을 불러오지 못했어요. 초대 코드는 직접 입력할 수 있어요.');
        }
      })
      .finally(() => {
        if (!ignore) {
  ...
  const handleShareInvite = async () => {
    ...
    try {
      ...
      setMessage(result === 'copied' ? '초대 메시지를 클립보드에 복사했어요.' : '초대 메시지를 공유했어요.');
    } catch {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      setJoinDisplayName('');
      setJoinInviteCode('');
      setMessage('공동 여행방에 참여했어요. 이제 현재 곡을 Recap 후보로 올릴 수 있어요.');
    } catch {
      setMessage('초대 코드로 여행방에 참여하지 못했어요. 코드를 다시 확인해주세요.');
    } finally {
  ...
  const handleToggleMomentStatus = async (moment: TravelRoomMoment) => {
    ...
    try {
      ...
      setMessage(nextStatus === 'accepted' ? '후보 Moment를 채택했어요.' : '후보 상태로 되돌렸어요.');
    } catch {
  ...
  const handleSubmitMomentComment = async (moment: TravelRoomMoment) => {
    ...
    try {
      ...
      setTravelRoom(sessionId, appendRoomMomentComment(room, moment.id, comment));
      ...
      <View className="mt-4 flex-row items-center gap-2">
          ...
          ? visibleMembers?.map((member, index) => (
              <View
                className={`h-9 w-9 items-center justify-center rounded-full border border-white/15 ${
                  member.userId === currentUserId ? 'bg-soundlog-lime' : 'bg-white/10'
                }`}
                key={member.id}
              >
                <AppText
                  className={`text-xs font-semibold ${
                    member.userId === currentUserId ? 'text-soundlog-inverse' : 'text-white'
                  }`}
                >
        ...
        {memberOverflowCount > 0 ? (
          <View className="h-9 items-center justify-center rounded-full border border-white/15 bg-white/10 px-3">
            <AppText className="text-xs font-semibold text-white">+{memberOverflowCount}</AppText>
          </View>
        ) : null}
        <AppText className="ml-1 flex-1 text-xs text-white/45" numberOfLines={2}>
          {room && visibleMembers?.length
            ? visibleMembers.map((member) => getMemberCaption(member, currentUserId)).join(' · ')
            : '초대 코드로 함께 참여'}
        </AppText>
      </View>
        ...
        {allCandidateMoments.length > COLLAPSED_MOMENT_COUNT ? (
          ...
          >
            <AppText className="text-xs font-semibold text-white/70">
              {showAllMoments ? '후보 접기' : `후보 ${hiddenMomentCount}개 더 보기`}
            </AppText>
          </Pressable>
        ) : null}
      </View>
