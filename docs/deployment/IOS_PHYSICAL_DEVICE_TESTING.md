...
DNS 오류나 타임아웃이 발생하면 배포 API 테스트를 중단하고 로컬 LAN API를 사용하거나 도메인·EC2 상태를 먼저 복구합니다. 도메인 장애 중에 EC2 IP를 앱에 임시 하드코딩하지 않습니다.
...
- `soundlog.shop` 사용 시 DNS와 Vercel rewrite, EC2 API 상태를 확인합니다.
